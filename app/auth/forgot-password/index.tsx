...
import { usePasswordResetMutation } from "@/redux/features/user/userApi";
...
export default function ForgotPasswordScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const regex = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i;
    if (!email) {
      setEmailError(trans("emailRequired") as string);
      return;
    }
    if (!regex.test(email)) {
      setEmailError(trans("invalidEmail") as string);
      return;
    }

    try {
      const options = { data: { email } };
      const result: any = await passwordReset(options);
      // console.log(result);
      // ✅ Success
      if (result?.data?.success) {
        setSuccess(true);
        toast.show(trans("successfull"), { type: "success" });
        return;
      }

      // ❌ Error from server (known)
      const errorResponse = result?.error?.data || result?.data;
      if (!errorResponse) throw new Error("Unknown error");

      switch (errorResponse?.status) {
        case 203:
          setServerError(trans("invalidPayload") as string);
          break;
        case 404:
          if (errorResponse?.type === "email") {
            setServerError(trans("userNotFound") as string);
          } else {
            toast.show(trans("userNotFound"), { type: "danger" });
          }
          break;
        case 201:
        default:
