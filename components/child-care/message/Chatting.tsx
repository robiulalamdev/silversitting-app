...
export default function ChattingScreen({ receiverId }: IProps) {
  ...
  useEffect(() => {
    if (conversationId && receiverId && user?._id && currentAllMessages) {
      putMessageSeen({ senderId: receiverId, conversationId: conversationId });
    }
  }, [currentAllMessages, receiverId]);
  ...
  useEffect(() => {
    if (conversationSuccess && conversationData._id) {
    ...
    try {
      ...
      if (res.data) {
        ...
        if (isReceiverOnline(targetReceiverId)) {
          ...
          await fetch(`${BASE_URL}/api/conversation/send-email-notification`, {
            method: "POST",
            headers: headers,
            body: JSON.stringify({
              receiverId: targetReceiverId,
