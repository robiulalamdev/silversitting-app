...
export default function ChildCareUi() {
  ...
  const handleSearchFormSubmit = async () => {
    ...
    if (!currentCity) {
      if (user?.residance) {
        Alert.alert(trans("useLocationPrompt") as string, "", [
          // Alert.alert("Use Current Location?", trans("useLocationPrompt"), [
          {
            text: trans("no") as string,
            style: "cancel",
            onPress: () => {
              setSearchError(trans("addressPrompt") as string);
            },
          },
          {
            text: trans("yes") as string,
            onPress: () => {
        ...
        setSearchError(trans("addressLoginPrompt") as string);
        return;
      }
    }
          ...
          <View className="bg-[#fcf9fc] rounded-lg p-6 relative">
            ...
            <View className="mb-6">
              {[
                ...
                <TouchableOpacity
                  key={index}
                  onPress={() => handleOfferProvideValue(offer.label as string)}
                  className="flex-row items-center py-2"
                >
                  <View
                    className={`w-5 h-5 border-2 rounded mr-2 ${
                      offers.includes(offer.label as string)
                        ? "bg-primary border-primary"
                        : "border-gray-400"
                    }`}
                  >
            ...
            <TouchableOpacity
              ...
              disabled={isLoading}
              className={`bg-primary rounded-lg py-4 px-6 max-w-[150px] mx-auto w-full ${isLoading ? "opacity-50" : ""}`}
      ...
      {step === "error" && (
        <View className="flex-1 p-6">
          ...
          >
            ...
            <Text className="text-gray-600 text-base text-center pb-5 mb-6">
              {trans("increaseAreaMessage")}{" "}
              <Text className="text-blue-500 underline" onPress={openModal}>
                {trans("clickHereText")}
              </Text>{" "}
              {trans("notifyMessage")}
            </Text>
