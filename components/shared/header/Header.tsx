...
import { useGetUnreadStatusQuery } from "@/redux/features/chat/chatApi";
...
export default function Header() {
  const { setRedirectPath, isAuthenticated, user } = useAuth();
  const { handleShowMenu } = useGlobal();
  const router = useRouter();

  const { data } = useGetUnreadStatusQuery(!user?._id ? skipToken : user._id);
    ...
    <>
      <View style={styles.header}>
        ...
        <View style={styles.rightSection}>
          ...
          <TouchableOpacity
            onPress={handleProBox}
            style={{ marginRight: 20 }}
            className="relative"
          >
            <MaterialIcons name="message" size={24} color={COLORS.primary} />
            {data && data?.unreadAvailable === true && (
              <View
                style={{
                  backgroundColor: "#09f809",
                  borderRadius: 8,
                  width: 8,
                  height: 8,
                  position: "absolute",
