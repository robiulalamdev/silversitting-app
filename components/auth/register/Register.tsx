...
export default function Register() {
  ...
  const validatePassword = (value: string) => {
    ...
    return passwordRegex.test(value) || trans("passwordValidationMessage");
  };
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    if (data.password !== data.rePassword) {
      toast.show(trans("passwordNotMatched"), { type: "danger" });
      return;
    }

    if (registerState.role === "childcarer" && !data.older) {
      toast.show(trans("confirmAgeRequirement"), {
        type: "danger",
      });
      return;
    }
    ...
    if (!data.terms || !data.privacy) {
      toast.show(trans("acceptTermsAndPrivacy"), {
        type: "danger",
      });
      return;
    }
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        toast.show(trans("activationLinkSent"), {
          type: "success",
        });
      } else {
        // Handle API specific errors returned in the data object but not caught by unwrap()
        toast.show(result.message || trans("somethingWentWrong"), {
          type: "danger",
        });
      }
    } catch (apiError: any) {
    ...
    <ScrollView className="flex-1 bg-white px-6">
      <View className="py-16">
          ...
          <View className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            {warningShow && (
              <Text className="text-sm text-gray-600 mt-2">
                {trans("childmindersAgeRequirement")}
              </Text>
            )}
          </View>
        )}
        ...
        <View className="gap-4 mb-6">
          <View className="flex-1">
            <Text className="text-gray-700 text-base mb-2">
              {trans("firstNameshort")} <Text className="text-red-500">*</Text>
            </Text>
            <Controller
              control={control}
              name="firstName"
              rules={{ required: trans("PleaseFillOut") }}
          ...
          <View className="flex-1">
            <Text className="text-gray-700 text-base mb-2">
              {trans("lastNameshort")} <Text className="text-red-500">*</Text>
            </Text>
            <Controller
              control={control}
              name="lastName"
        ...
        <View className="flex-row gap-4 mb-6">
          <View className="flex-1">
            <Text className="text-gray-700 text-base mb-2">
              {trans("postCodeshort")} <Text className="text-red-500">*</Text>
            </Text>
            <Controller
              control={control}
              name="postCode"
          ...
          <View className="flex-1">
            <Text className="text-gray-700 text-base mb-2">
              {trans("residence")} <Text className="text-red-500">*</Text>
            </Text>
            <Controller
              control={control}
              name="residence"
        ...
        <View className="gap-4 mb-6">
          ...
          <View className="flex-1">
            <Text className="text-gray-700 text-base mb-2">
              {trans("reEnterPassword")} <Text className="text-red-500">*</Text>
            </Text>
            <Controller
              control={control}
              name="rePassword"
              rules={{
                required: trans("PleaseFillOut"),
                validate: (value) =>
                  value === password || trans("passwordNotMatched"),
              }}
              render={({ field: { onChange, onBlur, value } }) => (
                <TextInput
                  mode="outlined"
        ...
        <Text className="text-center text-gray-600 text-sm">
          {trans("registrationAssistance")} info@silversitting.com.{" "}
          {trans("happyToHelp")}
        </Text>
      </View>
    </ScrollView>
